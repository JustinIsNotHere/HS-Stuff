...
library(readxl)
library(janitor)
data <- read_excel("R Weather Graphic Input.xlsx", sheet = "Philadelphia")
data <- data %>%
  clean_names()
...
p <- ggplot(data, aes(newDay, avg)) +
  ...
  geom_linerange(past, mapping=aes(x=newDay, ymin=lower, ymax=upper), colour = "cyan2", alpha=10)

print(p)

p <- p + 
  geom_linerange(past, mapping=aes(x=newDay, ymin=avg_lower, ymax=avg_upper), colour = "cyan4")

print(p)

p <- p + 
  geom_line(present, mapping=aes(x=newDay, y=high_f, group=1)) +
  geom_vline(xintercept = 0, colour = "cyan3", linetype=1, size=1)

print(p)

p <- p + 
  geom_line(present, mapping=aes(x=newDay, y=low_f, group=1)) +
  geom_vline(xintercept = 0, colour = "cyan", linetype=1, size=1)
